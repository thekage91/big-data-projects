...
var parseAndSave= function () {
    ...
    let _movies = [];
    var moviesStream = fs.createReadStream(moviesPath, { start: 10000001, end: 100000000 }).pipe(movieParser.startParsing);

    console.log("\n\nParsing Genres");
    var genresStream = fs.createReadStream(genresPath, { start: 70000000, end: 80000007 }).pipe(genresParser.startParsing);
    
    console.log("\n\nParsing Acotors");
    var actorsStream = fs.createReadStream(actorsPath, { start: 10000001, end: 20000004 }).pipe(actorsParser.startParsing);
    
    console.log("\n\nParsing Directors");
    var directorsStream = fs.createReadStream(directorsPath, { start: 10000001, end: 100000003 }).pipe(directorParser.startParsing);
