...
describe('Query: Retrieve all movies acted by one actor', function () {
    ...
    describe('Version 0', function () {
                ...
                q.all(all_saved).then(() => {
                    ...
                    for(let i=0; i < 100; i++) {
                        ( () => {
                            ...
                            if(i<10)
                                all_saved.push(save_interface.save(0, {movie: util.fakeMovie(), actor: saved_actor_with_10_movie}));
                            all_saved.push(save_interface.save(0, {movie: util.fakeMovie(), actor: saved_actor_with_100_movie}));
                        })();
                    }
        ...
        it('Retrieves 100 movie', function (done) {
            ...
            query_interface.all_films_one_actor(0, saved_actor_with_100_movie)
                .then((movies) => {
    ...
    describe('Version 1', function () {
            ...
            q.all(all_saved).then(() => {
                ...
                for(let i=0; i < 100; i++) {
                    ( () => {
                        //let movie = util.fakeMovie();
                        if(i<10)
                            all_saved.push(save_interface.save(1, {movie: util.fakeMovie(), actor: saved_actor_with_10_movie}));
                        all_saved.push(save_interface.save(1, {movie: util.fakeMovie(), actor: saved_actor_with_100_movie}));
                    })();
                }
                q.all(all_saved).then(() => done())
            })
        ...
        it('Retrieves 1 movie', function (done) {
            ...
            query_interface.all_films_one_actor(1, saved_actor_with_1_movie)
                .then((movies) => {
                    movies.length.should.eql(1);
                    movies[0].title.should.eql(saved_movie_with_1_actor.title);
                    done();
